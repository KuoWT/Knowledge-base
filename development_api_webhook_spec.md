...
## 1. 文件目的
本文件定義知識庫系統的工程實作規格、元件責任、API 介面、GitLab Webhook 規範、Hermes 排程行為與同步任務流程，供後端、平台與維運團隊實作與驗證。

## 2. 系統目標
- GitLab 為唯一正式內容來源。
- 內容更新分為人工流程、Agent 協作流程、Hermes 排程補償流程。
...
### 5.2 排程補償流程
1. Hermes 依排程啟動檢查。
2. 檢查是否存在漏觸 webhook、失敗任務或需重新索引項目。
3. 若需要，進入與正常同步相同的處理管線。
4. 若不需要，僅記錄檢查結果後結束。
...
### 14.3 排程錯誤
- 排程失敗不應影響 webhook 主流程
- 排程只作為補償與校正，不可覆蓋正式版本規則
